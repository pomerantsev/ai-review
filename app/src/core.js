...
export async function handle({ headers, body }) {
	...
	if (!payload.issue?.pull_request)
		return { status: 200, body: "ignored (not PR)" };

	const text = (payload.comment?.body || "").trim().toLowerCase();
	if (!text.startsWith("@pomerantsev-ai review"))
		return { status: 200, body: "ignored (no cmd)" };
	if (payload.comment?.user?.type === "Bot")
		return { status: 200, body: "ignored (bot)" };

	const appId = process.env.APP_ID;
	const privateKey = process.env.PRIVATE_KEY; // PEM string
